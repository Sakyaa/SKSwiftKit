...
public class SKScrollSegmentView: UIView {
    
    // 1. 实现颜色填充效果
    
    
    /// 所有的title设置(style setting)
    public var segmentStyle: SKSegmentStyle
    ...
    //MARK:- life cycle
    public init(frame: CGRect, segmentStyle: SKSegmentStyle, titles: [String]) {
    ...
    @objc func titleLabelOnClick(tapGes: UITapGestureRecognizer) {
        ...
        adjustUIWhenBtnOnClickWithAnimate(animated: false)
...
extension SKScrollSegmentView {
    ...
    public func reloadTitlesWithNewTitles(titles: [String]) {
        ...
        selectedIndex(selectedIndex: 0, animated: false)
    }
}
...
extension SKScrollSegmentView {
    private func setupTitles() {
        for (index, title) in titles.enumerated() {
            ...
            let tagGes = UITapGestureRecognizer.init(target: self, action: #selector(self.titleLabelOnClick(tapGes:)))
            label.addGestureRecognizer(tagGes)
            
            let text = title as NSString
            let size = CGSize(width:CGFloat.greatestFiniteMagnitude, height:0)
            let labelSize = text.boundingRect(with:size, options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font: label.font], context: nil)
            titlesWidthArray.append(labelSize.width)
