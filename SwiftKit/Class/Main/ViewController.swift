...
//  RACDemo
//
//  Created by Sakya on 2017/12/13.
...
class ViewController: OSViewController {
    ...
    //自定义的customNavigationview
    var customNavigationBar:CustomNavigationBarView!
    
    open func navigationBarStyleConfig(navigationBarStyle:CustomNavigationBarStyle,configuration:CustomNavigationBarConfig) {
        switch navigationBarStyle {
        case .isHideNavigationBar:
            break
        default:
            let customNavigationBar:CustomNavigationBarView = CustomNavigationBarView(style: navigationBarStyle, configuration: configuration, frame: CGRect.zero)
            customNavigationBar.delegate = self
            self.customNavigationBar = customNavigationBar
            view.addSubview(customNavigationBar)
            customNavigationBar.size = CGSize(width:KScreenConstant.sk_kScreenWidth, height:customNavigationBar.navigationBarHeight)
            customNavigationBar.manualLayoutSubviews()
            break
        }
    ...
    override func viewDidLoad() {
        view.backgroundColor = color_gray_background
        #if TRACE_RESOURCES
...
extension ViewController:CustomNavigationBarDelegate {
    func navigationBarTapEventDetected() {
    
    }
    func navigationBarRightButtonAction() {
        
    }
    func navigationBarMidViewAction() {
        
    }
    func navigationBarSecondRightButtonAction(){
...
extension ViewController {
    func initTableView()-> UITableView {
        let barHeight:CGFloat = self.customNavigationBar == nil ? 0.0 : self.customNavigationBar.navigationBarHeight
        
        let tableView = UITableView(frame: CGRect(x:0,y:barHeight,width:KScreenConstant.sk_kScreenWidth,height:KScreenConstant.sk_kScreenHeight - barHeight), style: .plain)
